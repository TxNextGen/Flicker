...
      } else {
        // MODIFICATION: If not using marked/DOMPurify, directly append text node
...
      // Ensure title updates correctly even if message is empty (e.g., image only)
      if (text) {
        this.allChats[this.currentChatId].title = text.length > 30 ? 
          text.substring(0, 30) + "..." : text;
      } else if (this.selectedImage) {
        this.allChats[this.currentChatId].title = "Image Shared";
      }
    }
...
      // Update: Set the new chat title based on message or "Image"
      const newId = this.createNewChat(message || (this.selectedImage ? "Image" : "New Chat"));
      if (!newId) return;
    }
...
    
    if (message) {
      // MODIFIED: Append text directly as a text node to userDiv
      userDiv.appendChild(document.createTextNode(message));
    }

    this.elements.chat.appendChild(userDiv);
    this.scrollToBottom();
    // MODIFIED: Correctly save message for image-only or text messages
    this.saveMessage("user", message || (this.selectedImage ? "Image shared" : ""));
...
    // MODIFIED: Add 'typing-indicator' class
    aiDiv.className = "message bot typing-indicator";
...
        // MODIFIED: Remove 'typing-indicator' class when reply is complete
