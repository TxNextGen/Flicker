...
<h1 class="text-xl font-semibold text-white {sidebarCollapsed ? 'hidden' : ''}">Flicker</h1>
</div>
<button
class="text-gray-400 hover:text-white transition-colors p-2 rounded-md hover:bg-[#1e1e3a]"
...
<button
class="w-full text-left px-3 py-2 rounded-md text-sm text-gray-300 hover:bg-[#1e1e3a] transition-colors truncate"
...
<button
class="text-gray-400 hover:text-blue-400 text-xs p-1"
...
{:else if currentUser}
<div class="text-gray-300 text-sm mb-3 px-2">Welcome, {currentUser.username}</div>
<div class="space-y-2">
<button
...
<div class="mt-auto pt-4 border-t border-[#1e1e3a] flex flex-col items-center space-y-2">
{#if currentUser}
<button
